...
import { getHintBottomOffsetClass, getHintBaloonVerticalPosClass, getHintBaloonHorizontalPosStyle } from '../helpers/GoogleMap/BalloonPos.js';
import { getMarkerHolderStyle, getMarkerStyle, getMarkerTextStyle } from '../helpers/GoogleMap/MarkerStyles.js';
...
function calcMarkerTextStyle(scale, markerTextStyle) {
  ...
  const colorV = Math.ceil(
    K_MIN_COLOR_VALUE +
    (K_MAX_COLOR_VALUE - K_MIN_COLOR_VALUE) * Math.min(scale / K_SCALE_NORMAL, 1)
  );
...
class PropertyMarker extends Component {
  ...
  render() {
    ...
    const showHint = this.props.hoverState ||
      this.props.showBallonState; // || this.props.hoveredAtTable;
...
export default controllable(['hoverState', 'showBallonState'])(PropertyMarker);
